...
├── data/                 # Where the raw sequencing data (barcode77.fastq) lives.
...
**Note:** The `barcode77.fastq` file got analyzed but due to Github's push limitations
the only compressed (.gz) form of data pushed to the Github. However, as mentioned before the results are 
the results of whole raw data. For running the pipeline please put the raw data in the actual form (.fastq),
<u>NOT</u> compressed form.
